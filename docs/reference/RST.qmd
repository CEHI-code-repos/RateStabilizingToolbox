...
>
>   <code>[**Rate Per**]{.parameter-name} [:]{.parameter-annotation-sep} [integer]{.parameter-annotation}</code>
> 
>   :   Output rate denominator (i.e. per 100,000).
>
>   <code>[**Number of Years**]{.parameter-name} [:]{.parameter-annotation-sep} [integer]{.parameter-annotation}</code>
> 
>   :   Number of years represented by input data (ouput rates will be divided by this number to give annual rates).
>
>   <code>[**Iteration Length**]{.parameter-name} [:]{.parameter-annotation-sep} [string]{.parameter-annotation}</code>
> 
>   :   Iteration length for the Gibbs sampler. Increasing the number of iterations ensures better convergence, thus improving the accuracy of the posterior estimates. Medium and high iteration settings can take 10+ minutes on large datasets (1,000+ records).
...
for one year of Michigan event data. I already took that data and ran it through the [Individual Data Processor](IDP.qmd). 
...
I want these rates to be per 100,000 with credible intervals at the 0.95 credible level for one year of Michigan
event data.
