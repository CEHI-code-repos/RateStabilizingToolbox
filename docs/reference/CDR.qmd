...
<code>[**API Key**]{.parameter-name} [:]{.parameter-annotation-sep} [string]{.parameter-annotation}</code>

:   (Required on first use) Census API Key. Request an API Key from the Census [here](https://api.census.gov/data/key_signup.html). The API Key will be stored within a `census_api_key.txt` file within the same directory as `rate_stabilizing_toolbox.pyt`.
